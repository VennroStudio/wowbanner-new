...
export const ClientTableRow: React.FC<Props> = ({ client, onEdit, onDelete }) => {
    const fullName = `${client.last_name} ${client.first_name} ${client.middle_name || ''}`.trim();
    const isJuridical = client.type.label.toLowerCase().includes('юр');
    ...
    return (
        <tr className="border-b border-slate-100 last:border-0 hover:bg-slate-50/70 transition-colors group">
            ...
            {/* Клиент */}
            <td className="px-5 py-4 align-top">
                <div className="font-medium text-slate-900 text-sm leading-snug mb-1.5 group-hover:text-blue-600 transition-colors cursor-pointer">
                    {fullName}
                </div>
                ...
                {client.companies && client.companies.length > 0 ? (
                    <div className="flex flex-col gap-1.5">
                        {client.companies.map((company) => (
                            <span
                                key={company.id}
                                className="inline-block px-2.5 py-1 bg-slate-100 text-slate-600
                           text-xs rounded font-medium w-fit"
                            >
