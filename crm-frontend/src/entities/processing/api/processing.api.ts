...
export const processingApi = {
  ...
  getProcessing: async (id: number | string) => {
    const { data } = await apiClient.get<ApiDataResponse<Processing>>(API_ENDPOINTS.PROCESSINGS.BY_ID(id));
    return data;
  },
  ...
  createProcessing: async (body: CreateProcessingBody) => {
    const { data } = await apiClient.post<ApiMutationResponse>(API_ENDPOINTS.PROCESSINGS.CREATE, body);
    return data;
  },

  updateProcessing: async (id: number | string, body: UpdateProcessingBody) => {
    const { data } = await apiClient.patch<ApiMutationResponse>(API_ENDPOINTS.PROCESSINGS.UPDATE(id), body);
    return data;
  },

  deleteProcessing: async (id: number | string) => {
    const { data } = await apiClient.delete<ApiMutationResponse>(API_ENDPOINTS.PROCESSINGS.DELETE(id));
    return data;
  },
  ...
  uploadImages: async (processingId: number | string, files: File[], imageAlts: string[]) => {
    ...
    const { data } = await apiClient.post<ApiMutationResponse>(API_ENDPOINTS.PROCESSINGS.IMAGES(processingId), formData);
    return data;
  },

  updateImageAlt: async (imageId: number | string, alt: string) => {
    const { data } = await apiClient.patch<ApiMutationResponse>(API_ENDPOINTS.PROCESSINGS.IMAGE_UPDATE(imageId), { alt });
    return data;
  },

  deleteImage: async (imageId: number | string) => {
    const { data } = await apiClient.delete<ApiMutationResponse>(API_ENDPOINTS.PROCESSINGS.IMAGE_DELETE(imageId));
    return data;
  },
};
