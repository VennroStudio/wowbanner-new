...
`shared/lib` использовать для:

- reusable hooks
- helpers с прикладным UI/domain-смыслом
- преобразований, которые напрямую обслуживают формы, таблицы, modal-flow, поиск, отображение, пользовательский ввод

Если helper выражает повторяемый UI/domain pattern, его место обычно в `shared/lib`.

#### Правило `shared/utils`

`shared/utils` использовать только для:

- маленьких технических pure-функций
- low-level helpers без локального состояния
- утилит, которые не являются готовым UI/domain pattern

Если helper больше похож на инфраструктурную или техническую функцию, чем на reusable UI primitive, его место в `shared/utils`.

Агенту запрещено раскладывать новые shared-файлы “по ощущению”. Перед созданием файла он обязан определить, это `lib` или `utils` по правилам выше.
...
## 9. Правила для таблиц

Если агент меняет или создаёт таблицу, он обязан использовать текущие shared-примитивы:

- `TableStateRow` — для empty/error состояний
- `RowActionButtons` — для action-кнопок в строках

Нельзя заново копировать:

- одинаковые empty-state строки
- одинаковые error-state строки
- одинаковые кнопки `Редактировать / Удалить`

## 10. Когда можно создавать новый shared primitive
...
## 11. Как добавлять новую сущность
...
## 12. Как добавлять новую страницу
...
## 13. Как продолжать проект, не ломая структуру
...
## 14. Правила для документации

Если агент ввёл новый устойчивый паттерн, он обязан:

1. обновить `README.md`
2. при необходимости обновить `AGENTS.md`

Агенту запрещено:

- полностью переписывать `README.md`, если задача была только на дополнение
- удалять базовые разделы запуска, Docker, инфраструктуры и proxy без отдельной причины
- вносить архитектурные изменения без отражения их в документации

## 15. Что сейчас особенно важно не ломать
...
7. shared table primitives в `shared/ui`
8. текущее разделение `shared/lib` и `shared/utils`

## 16. Финальный критерий любой задачи
