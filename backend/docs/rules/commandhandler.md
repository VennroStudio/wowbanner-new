...
**Исключение (подсущности):** Если сущность является логической частью родителя (one-to-many, которая могла бы быть полями в родительской таблице, например `{SubEntity}` для `{Entity}`), для неё **не создаются отдельные Action** (контроллеры/роуты).

- **Единый вход:** Все параметры для работы с подсущностью добавляются в **Action** (контроллер) и **Command** основной сущности. Необходимо дорабатывать Action основной сущности, чтобы он принимал и прокидывал эти поля.
- **Разделение логики:** Бизнес-логика подсущности остается в её **собственном Handler-е**, чтобы не раздувать основной Handler.
- **Вызов:** Handler основной сущности вызывает Handler подсущности, создавая для него его собственную Command.

**Пример ({Entity} + {SubEntity}):**
1. **Action:** `v1/{Entity}/Create{Entity}Action` теперь принимает поля как для `{Entity}`, так и для `{SubEntity}`.
2. **Command:** `Create{Entity}Command` содержит все эти поля.
3. **Handler:** `Create{Entity}Handler` после сохранения `{Entity}` делает вызов:
   ```php
   $this->Сreate{SubEntity}Handler->handle(new Create{SubEntity}Command(
       {Entity}Id: $entity->id,
