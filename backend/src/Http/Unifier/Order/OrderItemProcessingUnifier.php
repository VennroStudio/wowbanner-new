...
final readonly class OrderItemProcessingUnifier implements UnifierInterface
{
    #[Override]
    public function unifyOne(?int $userId, ?object $item): array
    {
        if (!$item instanceof OrderItemProcessingModelInterface) {
            return [];
        }
    ...
    {
        return UnifierHelper::toArrayWithout($item, 'order_item_id');
    }
}
