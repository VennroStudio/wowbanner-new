...
final readonly class ProductMaterialUnifier implements UnifierInterface
{
    #[Override]
    public function unifyOne(?int $userId, ?object $item): array
    {
        if (!$item instanceof ProductMaterialModelInterface) {
            return [];
        }
    ...
    {
        /** @var ProductMaterialModelInterface $item */
        return $item->toArray();
    }
}
